function Configure({Path}, opts) {
  ...
  var setCommandOptions = (cfg) => {
    ...
    if (cfg.logFlag) process.env[`LOG_${cfg.logFlag}`] = 'white'
    if (cfg.verbose) process.env[`LOG_APP_VERBOSE`] = 'white'
    else if (cfg.terse) process.env[`LOG_APP_TERSE`] = 'white'
  ...
  var setDefaults = (cfg) => {
    ...
    paths.slang = paths.slang ? Path.normalize(`${root}/${paths.slang}`) : './scream.slang.jk'
    ...
    return Object.assign(cfg, {setup, colors, paths, stubs, http, mocha})
  }


  var inValid = validate(instance)
  if (inValid) throw Error(`SCREAM config: ${inValid.white} at ${file}`)
