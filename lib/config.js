function Config({Path,Util}, root, appConfig, opts) {
  ...
  var cfgFile = opts.cmd && opts.cmd.config ? opts.cmd.config : 'scream.json'
  var cfg = require(Path.normalize(`${root}/${cfgFile}`))
  ...
  if (cmd.specs)
    cfg.specs = cmd.specs
  if (cmd.muted && appConfig) {
    cfg.muted = true
    appConfig.log = { app: { error: 'red' } }
  }
  if (cmd.verbose) {
    cfg.verbose = true
    if (appConfig && appConfig.log) appConfig.log.test = { verbose: true }
    global.verboseErrHandler = true
  }
  if (cmd.logFlag) {
    process.env[`LOG_${cmd.logFlag}`] = 'white'
    if (appConfig && appConfig.log)
      Util.Obj.set(appConfig.log, cmd.logFlag.toLowerCase().replace('_','.'), 'white')
  }
