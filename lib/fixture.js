...
module.exports = {
  ...
  init({Path, Fs, Util}, opts) {
    var cfg = opts.config.paths.fixtures
    if (!opts.config.paths.fixtures) return
    ObjUtil = Util.Obj
    Fs.readdirSync(cfg).forEach(file => {
      if (file.match(/(\.(js|coffee)$)/)) {
        var fixtureName = file.split('.')[0]
        try {
          var rawFixture = require(Path.join(cfg,file))
        } catch (e) {
