...
module.exports = {
  ...
  init({Path, Fs, Util}, {Config}) {
    if (!Config.paths.fixtures) return
    ObjUtil = Util.Obj
    Fs.readdirSync(Config.paths.fixtures).forEach(file => {
      if (file.match(/(\.(js|coffee)$)/)) {
        var fixtureName = file.split('.')[0]
        try {
          var rawFixture = require(Path.join(Config.paths.fixtures,file))
        } catch (e) {
