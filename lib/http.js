module.exports = function({}, {config}) {
  ...
  var _call = (httpCall, opts, cb) => {
    ...
    if (config.verbose) console.log(`${httpCall.method.toUpperCase()}\t`.cyan,
                                     httpCall.url.replace('http://','').cyan)
    ...
    httpCall
      ...
      .end(function(e, resp) {
        if (e) {
          console.log(`[${resp.request.method.toUpperCase()}${resp.request.req.path}]${e.message}`.gray)
          if (config.verbose) console.log(`--response (${resp.type})\n`.white, resp.res.body||resp.res.text, '\n--\n'.white)
          // global.DONE = (error) => DONE(error || e)
          // return cb.call(this)
          return DONE(e)
        }
        try {
  ...
  var apiUrl = config.http.api.baseUrl || '/api'
