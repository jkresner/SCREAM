module.exports = function(cfg) {
  ...
  var _call = (httpCall, opts, cb) => {
    ...
    httpCall
      ...
      .end(function(e,resp) {
        if (e) {
          // console.log(resp.res.body||resp.res.text)
          return DONE(e)
        }
        try {
  ...
  global.GET = (url, opts, cb) =>
    _call(http(global.APP).get(`${apiPrefix}${url}`), opts, cb)

  global.POST = (url, data, opts, cb) =>
    _call(http(global.APP).post(`${apiPrefix}${url}`).send(data), opts, cb)

  global.PUT = (url, data, opts, cb) =>
    _call(http(global.APP).put(`${apiPrefix}${url}`).send(data), opts, cb)

  global.DELETE = (url, opts, cb) =>
    _call(http(global.APP).delete(`${apiPrefix}${url}`), opts, cb)
