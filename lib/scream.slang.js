...
chai.Assertion.addChainableMethod('bsonId', function() {
  var input = this.__flags.object
  this.assert(input != null, `expect(<BSONID>) requires ObjectID to chain expect.bsonId but got null`)
  var mongoId = input instanceof ObjectId
  var mongooseId = `${input}`.length == 24 && input.constructor != String
  this.assert(mongoId || mongooseId, `expect(<BSONID>) but got constructor <${typeof input.constructor}> for ${input}`)
})
