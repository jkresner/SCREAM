module.exports = (db) => ({
  ...
  testToSeed(cb) {
    ...
    if (cfg.seed.force) {
      OPTS.log.setup('SEEDING', 'forced')
      return cb(null, true)
    }

    // mode == 'FIXTURE' only for now
    var [mode,fixtureName,key] = cfg.seed.without.split('.')
    OPTS.log.setup('DBSEED', `without FIXTURE.${fixtureName}.${key}`)
    ...
    collection.findOne({_id:seed._id}, (e,r) => {
      if (e) throw e
      OPTS.log.setup('SEEDING', r==null ? `${seed.dirty===false?"clean/overwrite":"dirty/upsert"}` : false)
      cb(r==null)
    })
  },
