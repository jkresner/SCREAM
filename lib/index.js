  ...
  return {
    ...
    run(appOpts) {
      ...
      new Mocha(Object.assign({bail:true}, opts.cmd))
        ...
        .on ('fail', f => {
          // console.log('MOCHA.Fail'.yellow, f, f.async)
          // console.log(f.err.stack)
          // console.log(`SCREAM Spec.IT fail =>`.gray,
          //   _.take(f.err.stack.split('\n').filter(ln => !ln.match(new RegExp(opts.config.init.fail.filter,'i'))), 15)
          //    .map(ln => ln.red)
          //    .join('\n'), '\n\n')
          if (f.async != 1) process.exit(f)
        })
        .on ('end', f => {
