...
global.expectExactFields = (obj, fieldsArray, moreFields) => {
  ...
  for (var name of fieldsArray)
    expect(_.has(obj,name), `obj missing attr ${name.white}`).to.be.true

  for (var name of _.keys(obj))
    expect(_.contains(fieldsArray,name), `extra attr ${name.white} on` + JSON.stringify(obj).gray).to.be.true
